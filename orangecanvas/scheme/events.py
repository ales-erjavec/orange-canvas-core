...
class NodeEvent(WorkflowEvent):
    ...
    def __init__(self, etype, node, pos=-1, parent=None):
        # type: (QEvent.Type, Node, int, Optional[MetaNode]) -> None
...
class LinkEvent(WorkflowEvent):
    ...
    def __init__(self, etype, link, pos=-1, parent=None):
        # type: (QEvent.Type, Link, int, Optional[MetaNode]) -> None
        super().__init__(etype)
        self.__link = link
        self.__pos = pos
        self.__parent = parent
...
class AnnotationEvent(WorkflowEvent):
    ...
    def __init__(self, etype, annotation, pos=-1, parent=None):
        # type: (QEvent.Type, Annotation, int, Optional[MetaNode]) -> None
        super().__init__(etype)
