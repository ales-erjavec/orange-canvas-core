...
def grayscale(src: QImage):
    format = QImage.Format_ARGB32_Premultiplied if src.hasAlphaChannel() else QImage.Format_RGB32
    image = src.convertToFormat(format)
    size = image.size()
    w, h = size.width(), size.height()
    buffer = image.bits().asarray(w * h * 4)
    view = np.frombuffer(buffer, np.uint32).reshape((w, h))
    r, g, b, a = qred(view), qgreen(view), qblue(view), qalpha(view)
    gray = qgray(r, g, b)
    rgba = qrgba(gray, gray, gray, a)
    assert rgba.shape == r.shape == view.shape
    res = QImage(w, h, format)
    buffer = res.bits().asarray(w * h * 4)
    view = np.frombuffer(buffer, np.uint32).reshape((w, h))
    view[:] = rgba
    return res
    return qimage_from_array(rgba)
